...
//Get any other veriaties
                    const isOtherVariates = new Promise ((resolve, reject) => {
                        if((otherVariates.length > 0) && (pokemonName !== 'pikachu')) {
                            var url = pokemon.gender_rate === pokemon.gender_rate === -1 
                            ? 'uk_n_00000000_f_n.png' 
                                : pokemon.gender_rate === 8
                                    ? 'fo_n_00000000_f_n.png' 
                                        :'mf_n_00000000_f_n.png';

                            if((otherVariates[0].pokemon.name === pokemonName.toLowerCase()) 
                                && (pokemonName !== 'calyrex') && (pokemonName !== 'rotom')
                                && (pokemonName !== 'hoopa') && (pokemonName !== 'kyurem')
                                && (pokemonName !== 'eternatus') && (pokemonName !== 'greninja')) {
                                otherVariates.shift()
                            }   
                            
                            if((otherVariates.length === 0) 
                                && !genderDif 
                                ...
                                normal.normal.push(
                                    {
                                        'type': 'normal',
                                        'url': `${pokemonImage(pokemon)}`,
                                        'details': pokemon
                                    }
                                )

                            } else if((pokemon.forms.length > 1) && (pokemonName !== 'pichu')) {
                                const formId = (id) => {
                                    var digitLen = id.toString().length;
                                    var newId = {
                                        1: `00${id}`,
                                    ...
                                    pokemon.forms.forEach((obj, i) => {
                                        normal.normal.push(
                                            {
                                                'type': `${prettyText(obj.name)}`, 
                                                'url': `${path}${pokemonId()}_${formId((i-1)+1)}_${url}`,
                                                'details': pokemon
                                            }
                                        )
                                    })
